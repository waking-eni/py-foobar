...
no matter how many times it continues iterating. Starting with n = 1211, the routine will reach the integer 6174, 
...
def backToOriginBase(number, base):
    convNum = []
    while number > 0:
        convNum.insert(0, str(number % base))
        number = number / base
    return ''.join(convNum)
...
def getSubZ(x,y,b):
    ...
    baseTenX = convertToBaseTen(x,b)
    baseTenY = convertToBaseTen(y,b)
    baseTenZ = baseTenX - baseTenY
...
def solution(n, b):
    ...
    while True:
        ...
        lenZ = len(str(z))
        if lenX != lenZ:
            z = z * pow(10, int((lenX - lenZ)))
        
        for index, value in enumerate(cycOfValues):
            if value == z:
                return index + 1
                break
        cycOfValues = [z] + cycOfValues
        n = z
