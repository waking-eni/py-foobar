...
def solution(total_lambs):
    ...
    while x <= total_lambs:
        currentvalue = 2**x
        genereousList.append(currentvalue)
    ...
    y = 2
    while y <= total_lambs:
        value = stingyList[y - 1] + stingyList[y - 2]
        stingyList.append(value)
        fibRunningTotal = fibRunningTotal + int(stingyList[y])
        if fibRunningTotal > total_lambs:
            break
        y = y+1
    solution = len(stingyList) - len(genereousList)
    return abs(solution)
